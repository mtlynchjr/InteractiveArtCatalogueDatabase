...
def delete_artwork(deleted_artwork):
    # deleted_artwork = input("What is the name of the artwork you would like to remove? ")
    with sqlite3.connect(database) as conn:
...
def available_artwork(available_artist):
    #available_artist = input("What is the name of the artist whose available artworks you want to see? ")
    with sqlite3.connect(database) as conn:
